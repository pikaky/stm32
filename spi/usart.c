...
void usart_send4byte(USART_TypeDef* USARTx,uint32_t Data)
{
	...
	temp1= (Data&0xff000000)>>24;  //提取高8位并放到最左边 因为等等只会发送前面8个bit
	temp2= (Data&0x00ff0000)>>16; //提取8位
	temp3= (Data&0x0000ff00)>>8;
	temp4= (Data&0x000000ff)>>0;
	
	//先发送高8位 然后低8位
	USART_SendData(USARTx,temp1);
	while(USART_GetFlagStatus(USARTx,USART_FLAG_TXE)==RESET);	//判断发送完毕

	USART_SendData(USARTx,temp2);
	while(USART_GetFlagStatus(USARTx,USART_FLAG_TXE)==RESET);
	
	USART_SendData(USARTx,temp3);
	while(USART_GetFlagStatus(USARTx,USART_FLAG_TXE)==RESET);
	
	USART_SendData(USARTx,temp4);
	while(USART_GetFlagStatus(USARTx,USART_FLAG_TXE)==RESET);
}
...
{
	unsigned char i;
	unsigned char temph,templ,tempu,tempm; //定义一个高8位和低8位
	
	
	for(i=0;i<arrnumber;i++)
	{
	
	temph= (arr[i]&0xff000000)>>24;  //提取高8位并放到最左边 因为等等只会发送前面8个bit
	tempm= (arr[i]&0x00ff0000)>>16; //提取8位
	templ= (arr[i]&0x0000ff00)>>8;
	tempu= (arr[i]&0x000000ff)>>0;
	...
	//再发送低8位 然后低8位
	USART_SendData(USARTx,tempm);
	while(USART_GetFlagStatus(USARTx,USART_FLAG_TXE)==RESET);
	
	USART_SendData(USARTx,templ);
	while(USART_GetFlagStatus(USARTx,USART_FLAG_TXE)==RESET);	//判断发送完毕

	USART_SendData(USARTx,tempu);
	while(USART_GetFlagStatus(USARTx,USART_FLAG_TXE)==RESET);
